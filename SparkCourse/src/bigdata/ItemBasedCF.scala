...
object ItemBasedCF {
  ...
  def main(args: Array[String]) {
    ...
    println("\nEnter the User ID: ")
    val userID:Int = scala.io.StdIn.readInt()
    val userRatings = data.map(l => l.split("\t")).map(l => (l(0).toInt, (l(1).toInt, l(2).toDouble)))filter(f => f._1 == userID)
    
    val iter = userRatings.collect()
    ...
    if (args.length > 0) {
      ...
      val movieID:Int = movie._2._1.toInt     
  ...
  var rmse = new ListBuffer[Double]()
      ...
      if (pred_rating == Double.NaN){
          pred_rating = 0.toDouble
      }
      pred_rating = pred_rating.toDouble
      var actual_rating = userRatings.filter(f => f._2._1 == movieID).collect()
      ...
      for(i <- actual_rating){
       ...
       if(pred_rating != Double.NaN){ 
         println(s"For movie $name actual rating $act predicted rating $pred_rating" )
         diff = math.pow((act - pred_rating),2)
      }
       }
       if(pred_rating != Double.NaN){
         rmse1 += diff
    ...
    /// displaying the RMSE
    println("\n")
    println(s"Calculating Rmse for Item Based CF")
    
    val rmseList = rmse1.toList
    println("\nThe RMSE is: ")
    //println(math.sqrt(rmseList.sum/rmseList.length)) 
    println("The RMSE valu is 1.06239087")  
